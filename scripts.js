...
function initModalEvents() {
  ...
  if (openBtn) {
    openBtn.addEventListener('click', () => {
      ...
      // Render button if SDK is loaded and container is empty
      const container = document.querySelector('#paypal-container-modal');
      if (window.paypal && container && container.children.length === 0) {
         // The ID below acts as a fallback if the fetch fails or script loads via noscript/default
         renderPayPalButtons("H4Y8CJZL7NDNG"); 
      }
    });
...
// --- PayPal Integration ---
async function initPaypalIntegration() {
  try {
    // Try to fetch the client ID from the Cloudflare Pages Function
    const response = await fetch('/paypal-config');
    if (response.ok) {
      const config = await response.json();
      if (config.clientId) {
        loadPaypalScript(config.clientId);
      } else {
        console.warn('PayPal Client ID not found in config.');
      }
    } else {
      console.warn('Could not fetch PayPal config. Using fallback or hosted button directly.');
    }
  } catch (error) {
...
function loadPaypalScript(clientId) {
  ...
  script.onload = () => {
    ...
    // Optional: Pre-render if modal is already open? 
    // For now, we wait for user click on Donate button which triggers render.
  };
  document.body.appendChild(script);
}

function renderPayPalButtons(hostedButtonId) {
